...
// 1. Get Distinct Degrees from course_allocation table
const getDistinctDegrees = async () => {
    try {
        console.log('Fetching all degrees from course_allocation table...');
        const allData = await fetchAllRows(
            supabase.from('course_allocation').select('degree')
        );
...
// 4. Get Faculty for selected course (from course_feedback table)
// This matches course_code from course_allocation to course_feedback and fetches faculty details
const getFacultyByCourse = async (degree, department, courseCode, staffIdFilter) => {
    try {
        console.log(`\n=== Fetching Faculty for Course ===`);
        console.log(`Degree: ${degree}`);
        console.log(`Department/Staff Dept: ${department}`);
        console.log(`Course Code: ${courseCode}`);
        console.log(`Staff ID Filter: ${staffIdFilter || 'N/A'}`);

        // Primary match: course_code (links course_allocation to course_feedback)
        // Optionally filter by degree for better accuracy
        const cleanedCourseCode = courseCode.trim();
        let query = supabase
            .from('course_feedback')
            .select('faculty_name, staff_id, staffid, course_code, course_name, batch, degree, dept')
            .like('course_code', `${cleanedCourseCode}%`)
            .not('faculty_name', 'is', null)
            .not('course_code', 'is', null);

        // If degree is provided, filter by it for better accuracy
        if (degree) {
            query = query.eq('degree', degree);
        }

        // Apply degree filter early if provided
        if (degree) {
            query = query.eq('degree', degree);
        }
        ...
        if (allData.length === 0) {
            console.log('⚠️ No feedback data found in course_feedback table');
            return [];
        }

        // Data already filtered by DB; keep defensively
        const filteredData = allData.filter(item => {
            const itemCourseCode = cleanString(item.course_code);
...
// 4b. Get unique batches from course_feedback for a specific faculty+course combination
const getBatchesForFacultyCourse = async (courseCode, staffId) => {
    try {
        ...
        // Query course_feedback for batches matching course_code (prioritize staffid matching)
        ...
        const allData = await fetchAllRows(query);
        console.log(`Raw batch data from course_feedback: ${allData.length} rows`);
